class Wsgw::OutspokenController < ApplicationController
  ...
  def send_job
    ...
    t = Benchmark.realtime do
      ...
      response_delay = params[:delay] =~ /^\d{1,2}$/ ? params[:delay].to_i : 3 #(rand(20) + 5).to_f / 10
    ...
    puts "Processing Job #{@job_request_id}. Status: #{@sms_job_log.status_code}" if @job_request_id
