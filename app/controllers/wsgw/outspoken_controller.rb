class Wsgw::OutspokenController < ApplicationController
  ...
  def send_job
    t = Benchmark.realtime do
      ...
      @num_destinations = (params[:recipients] || 1).to_i
    ...
    if @sms_job_log = SmsJobLog.where(job_request_id: @job_request_id).order('created_at DESC').first
      ...
      @sms_job_log = SmsJobLog.create(job_request_id: @job_request_id, duration: t, status_code: status_code, num_destinations: @num_destinations)
    end
